...
class Port {
  public:
    Lock * lock;
    Condition* noReceiver;
    Condition* noSender;

    Port(int p) {
      portNum = p;
      noReceiver=new Condition("No Receiver");
      noSender=new Condition("No Sender");
      lock=new Lock("Lock");
      msg=NULL;
    }
    ...
    bool produceMsg(int _msg) {
        msg=_msg;
    }

    bool noReceiverCheck() {
      return msg!=NULL;
    }
...
//----------------------------------------------------------------------
// Declaration of Train problem
//----------------------------------------------------------------------

class Train {
  public:
    Train(int _name, int _currStation);
    bool UnloadPassengers();
    bool getNextPassengers();
    void MoveToNextStation();
    int get_num_passengers_riding();
    int name;
  private:
    int curr_station;
    int num_passengers;
    
    List* passengers_riding=new List();   // keeps track of passengers in the train
};
...
class Station {
  public:
    ...
    Station(int);
    void AddPassengerWaiting(Passenger * p);
    void* RemovePassengerWaiting();
    Passenger* remove_passenger_waiting_from_head();
    int get_num_passengers_waiting();
  private:
    // const int STATION_CAPACITY=100;
    int name;
    int num_passengers_waiting;
    Lock * add_passenger_lock; 
    List* passengers_waiting;  // stores passengers waiting at the station
};

class Passenger {
  public:
    Passenger(int name, int fromStation,int toStation);
